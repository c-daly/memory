...
- **Append-only.** No `delete()`, no `update()`. Corrections are new entries that consumers reconcile. Under v2 layout (`<entity>/.memory/<date>-<name>.md`), the `MemoryCollisionError` enforcement is on `(name, date)` per resolved-entity — same name on the same day under the same entity collides regardless of `type` (type is frontmatter, not part of the path). Use distinct names if you need same-day differentiation.
